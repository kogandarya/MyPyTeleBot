...
bot = telebot.TeleBot('5146663877:AAEX0qvSj1ugoarXZTMbj11WUivGuqqUdZ4')  # Создаем экземпляр бота
...
def send_help(chat_id):
    global bot
    bot.send_message(chat_id, "Автор: Коган Дарья")
    markup = types.InlineKeyboardMarkup()
    btn1 = types.InlineKeyboardButton(text="Напишите автору", url="https://t.me/kogandarya")
    markup.add(btn1)
    img = open('Коган Дарья.png', 'rb')
...
def get_news():
    array_news = []
    req_n = requests.get('https://www.banki.ru/news/lenta')
    if req_n.status_code == 200:
        soup = bs4.BeautifulSoup(req_n.text, "html.parser")
        result_find = soup.select('.NewsItemstyled__StyledItemTitle-sc-jjc7yr-7 bhaAOk')
        for result in result_find:
            print(result)
...
def get_fact():
    array_facts = []
    req_f = requests.get('http://www.potehechas.ru/club/fact_1.shtml')
    soup = bs4.BeautifulSoup(req_f.text, "html.parser")
    result_find = soup.select('.text_aforizm')
    for result in result_find:
